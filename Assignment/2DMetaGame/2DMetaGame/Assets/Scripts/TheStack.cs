...
public class TheStack : MonoBehaviour
{
    ...
    bool Spawn_Block()
    {
        ...
        newTrans.parent = this.transform;  // ?  새롭게 생성된 블록은 부모없이 오브젝트 생성이라 스택의 부모를 가져다준다 연결시켜주는거..?
    ...
    bool PlaceBlock()
    {
        ...
        if (isMovingX)   //델타 x 가 잘려나가야 하는 크기
        {
            float deltaX = prevBlockPosition.x - lastPosition.x;
            bool isNegativeNum = (deltaX < 0) ? true : false; // 러블을 생성하는 방향을 정해주는 것
